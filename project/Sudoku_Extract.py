...
class Sudoku_Get:
    ...
    def filtered_binarize(self):
        ...
        # cv2.imshow("binarized Image", self.closing)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        return self.binary
...
class Cell:
    ...
    # def predict_cells(cells, classifier):
    #     predictions = []
    #     for index, cell in enumerate(cells):
    #         marginX = int(cell.shape[1] * 0.2)
    #         marginY = int(cell.shape[0] * 0.15)
    #         trimmed_cell = cell[marginY:-marginY, marginX:-marginX]

    #         if index in [0, 2, 7, 17]:
    ...
    #         if white_area < 0.1 * total_area:
    #             print(white_area / total_area)
    #             predictions.append(0)
    #         else:
    #             preprocessed = Cell.preprocess_cell(trimmed_cell)
