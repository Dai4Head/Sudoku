...
class Sudoku_Get:
    ...
    def overlay_solution(self, original_sudoku, solution, original_img):
        cell_width = original_img.shape[1] // 6
        cell_height = original_img.shape[0] // 6
...
class Cell:
    ...
    def extract_cells(sudoku_img):
        stepX = sudoku_img.shape[1] // 6
        stepY = sudoku_img.shape[0] // 6
        cells = []
    ...
    def predict_cells(cells, model):
        ...
        for index, cell in enumerate(cells):
            ...
            if white_area < 0.05 * total_area:
                ...
                cell = cv2.resize(trimmed_cell, (28, 28))
    ...
    def assemble_sudoku_matrix(predictions):
        print(np.array(predictions).reshape(6, 6))
        return np.array(predictions).reshape(6, 6)
